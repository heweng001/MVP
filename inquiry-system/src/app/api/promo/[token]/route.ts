...
export async function POST(req: NextRequest, ctx: Ctx) {
  ...
  if (!isEditTokenValid(promo.editTokenExpires)) {
    return NextResponse.json({ error: "编辑链接已过期（超过 7 天），请联系管理员重新发送" }, { status: 403 });
  }
  ...
  return NextResponse.json({
    ok: true,
    lastSubmittedBy: updated.lastSubmittedBy,
    lastSubmittedAt: updated.lastSubmittedAt?.toISOString() ?? null,
  });
}
